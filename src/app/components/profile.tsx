...
const Profile = () => {
  const [user, setUser] = useState<{
    nom: string; age: string; classe: string; departement: string;
    formationChoisie: string; coursInscrits: string[]
  }>({
    nom: '', age: '', classe: '', departement: '',
    formationChoisie: '', coursInscrits: [],
  });
  ...
  useEffect(() => {
    ...
    if (storedUser.nom) setUser(storedUser);
  }, []);
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div className="bg-white rounded-lg shadow-lg overflow-hidden">
            ...
            <div className="px-6 pb-6">
              <input 
                ...
                onChange={(e) => {
                  ...
                  if (searchValue) {
                    const baseList = afficherTousCours ? coursDisponibles : 
                      (user.formationChoisie ? coursParFormation[user.formationChoisie as keyof typeof coursParFormation] : []);
                    setCoursFiltrés(baseList.filter(cours => cours.toLowerCase().includes(searchValue)));
                  } else {
                    setCoursFiltrés(afficherTousCours ? coursDisponibles : 
                      (user.formationChoisie ? coursParFormation[user.formationChoisie as keyof typeof coursParFormation] : []));
                  }
                }}
              />
            ...
            <div className="px-6 pb-6 grid grid-cols-1 gap-3 max-h-72 overflow-y-auto">
              {coursFiltrés.length > 0 ? coursFiltrés.map((cours) => (
                <button
                  key={cours}
